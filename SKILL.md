...
- **[금융업 특화 규칙]** 은행, 보험, 증권 등 금융주는 '매출액' 계정 대신 **'영업수익'**을, '영업이익' 대신 **'영업이익(손실)'** 계정을 탐색하여 매핑하십시오. 또한 당기순이익 산출 시 **'지배기업주주지분순이익'**을 우선 활용하여 PER 정합성을 높이십시오.

### Step 3: 종합 밸류에이션 및 기술적 분석
- 산출된 지표를 바탕으로 벤치마크 대형주들의 **산술 평균(Benchmark Average)**을 반드시 계산하십시오.
- 중소형주 분석 시 이 평균값 대비 저평가율(%)을 정밀 판정합니다.
- `google_web_search`를 통해 120일 이동평균선(120MA) 위치를 확인하여 기술적 타점을 분석합니다.

### Step 4: 마스터 리포트 생성 및 저장 (V5.5 Masterpiece)
반드시 아래의 **V5.5 Masterpiece HTML 템플릿**을 사용하여 리포트를 생성하고, 다음의 **저장 및 작성 규칙**을 엄격히 준수하십시오.

**[작성 가이드라인]**
- **데이터 일치:** 테이블의 `{{Benchmark_Rows}}`와 `벤치마크 평균` 행의 데이터가 상호 일치해야 합니다.
- **비교 분석:** `Value/Momentum Track` 카드 설명에 "벤치마크 평균 대비 X% 저평가"와 같은 구체적인 수치를 포함하십시오.

**[저장 및 명명 규칙]**
1.  **산업별 폴더 관리:** 각 산업(섹터)별로 독립된 폴더를 생성하여 관리합니다.
2.  **파일명 규격:** 파일명은 반드시 `[산업명]_master_report_v5.5_{YYMMDD}.html` 형식으로 작성합니다.
...
<body>
    <div class="master-wrap">
        ...
        <div class="section-title"><span style="color: var(--value);">3. Value Track:</span> 장기 가치 투자 Top 3</div>
        <div class="recommend-grid">
            {{Value_Cards}}
        </div>
        <div class="section-title"><span style="color: var(--momentum);">4. Momentum Track:</span> 단기 추세 매매 Top 3</div>
        <div class="recommend-grid">
            {{Momentum_Cards}}
        </div>
...
### Scripts
`scripts/fetch_dart_data.py`를 사용하여 데이터를 추출하십시오.
